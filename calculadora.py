...
def calcular():
    try:
        val_entrada = entrada.get()
        expr = val_entrada.replace('^', '**').replace('mod', '%')
        ...
        linea_historial = f"{val_entrada} = {res}"
        historial.insert(0, linea_historial)
        
        with open("historial.txt", "a") as f:
            f.write(linea_historial + "\n")
        ...
        messagebox.showerror("Error", "Operacion Invalida")
...
historial = tk.Listbox(pantalla_frame, height=3, font=("Consolas", 10), bg="#1E293B", fg="#94A3B8", borderwidth=0, highlightthickness=0)
...
btn_ui("CLR LST", 0, 2, borrar_historial, "#64748B", "white", s=1)
